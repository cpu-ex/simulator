...
decoder = {
    ...
    # pc rd, imm
    'LUI': (re.compile(r'(lui)\s+(\S+),\s+(-?\d+)'), 1),
    'AUIPC': (re.compile(r'(auipc)\s+(\S+),\s+(-?\d+)'), 1),
    # jal rd, tag
    'JAL': (re.compile(r'(jal)\s+(\S+),\s+(\S+)'), 1),
    # jalr rd, offset(rs1)
    'JALR': (re.compile(r'(jalr)\s+(\S+),\s+(-?\d+)\((\S+)\)'), 1),
    # branch rs1, rs2, tag
    ...
    # load rd, offset(rs1)
    'LB': (re.compile(r'(lb)\s+(\S+),\s+(-?\d+)\((\S+)\)'), 1),
    'LH': (re.compile(r'(lh)\s+(\S+),\s+(-?\d+)\((\S+)\)'), 1),
    'LW': (re.compile(r'(lw)\s+(\S+),\s+(-?\d+)\((\S+)\)'), 1),
    'LBU': (re.compile(r'(lbu)\s+(\S+),\s+(-?\d+)\((\S+)\)'), 1),
    'LHU': (re.compile(r'(lhu)\s+(\S+),\s+(-?\d+)\((\S+)\)'), 1),
    # store rs2, offset(rs1)
    'SB': (re.compile(r'(sb)\s+(\S+),\s+(-?\d+)\((\S+)\)'), 1),
    'SH': (re.compile(r'(sh)\s+(\S+),\s+(-?\d+)\((\S+)\)'), 1),
    'SW': (re.compile(r'(sw)\s+(\S+),\s+(-?\d+)\((\S+)\)'), 1),
    # arith_i rd, rs1, imm
    'ADDI': (re.compile(r'(addi)\s+(\S+),\s+(\S+),\s+(-?\d+)'), 1),
    'SLLI': (re.compile(r'(slli)\s+(\S+),\s+(\S+),\s+(-?\d+)'), 1),
    'SLTI': (re.compile(r'(slti)\s+(\S+),\s+(\S+),\s+(-?\d+)'), 1),
    'SLTIU': (re.compile(r'(sltiu)\s+(\S+),\s+(\S+),\s+(-?\d+)'), 1),
    'XORI': (re.compile(r'(xori)\s+(\S+),\s+(\S+),\s+(-?\d+)'), 1),
    'SRLI': (re.compile(r'(srli)\s+(\S+),\s+(\S+),\s+(-?\d+)'), 1),
    'SRAI': (re.compile(r'(srai)\s+(\S+),\s+(\S+),\s+(-?\d+)'), 1),
    'ORI': (re.compile(r'(ori)\s+(\S+),\s+(\S+),\s+(-?\d+)'), 1),
    'ANDI': (re.compile(r'(andi)\s+(\S+),\s+(\S+),\s+(-?\d+)'), 1),
    # arith rd, rs1, rs2
    ...
    'PSEUDO-LI': (re.compile(r'(li)\s+(\S+),\s+(-?\d+)'), 2),
