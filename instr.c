...
void LUI(CORE* core, INSTR instr) {
    BYTE rd = instr.u.rd;
...
void AUIPC(CORE* core, INSTR instr) {
    BYTE rd = instr.u.rd;
    WORD imm = instr.u.imm31_12;
...
// jal: jump and link
void JAL(CORE* core, INSTR instr) {
    BYTE rd = instr.j.rd;
...
void JALR(CORE* core, INSTR instr) {
    BYTE rd = instr.i.rd;
...
void BRANCH(CORE* core, INSTR instr) {
...
void LOAD(CORE* core, INSTR instr) {
    BYTE rd = instr.i.rd;
    WORD imm = instr.i.imm;
    BYTE rs1 = instr.i.rs1;
...
// store: sb, sh, sw
void STORE(CORE* core, INSTR instr) {
...
void ARITH_I(CORE *core, INSTR instr) {
    BYTE rd = instr.i.rd;
    WORD imm = instr.i.imm;
    BYTE rs1 = instr.i.rs1;
    BYTE funct3 = instr.i.funct3;
...
void ARITH(CORE *core, INSTR instr) {
    BYTE rd = instr.r.rd;
...
void execute(CORE* core, INSTR instr) {
    ...
    /* risc-v I */
    // lui
    case 0b0110111: LUI(core, instr); break;
    // auipc
    case 0b0010111: AUIPC(core, instr); break;
    // jal
    case 0b1101111: JAL(core, instr); break;
    // jalr
    case 0b1100111: JALR(core, instr); break;
    // branch
    case 0b1100011: BRANCH(core, instr); break;
    // load
    case 0b0000011: LOAD(core, instr); break;
    // store
    case 0b0100011: STORE(core, instr); break;
    // arith I
    case 0b0010011: ARITH_I(core, instr); break;
    // arith
    case 0b0110011: ARITH(core, instr);break;
    // fence
    case 0b0001111:
    // env + csr
    case 0b1110011:
